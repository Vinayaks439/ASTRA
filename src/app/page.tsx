...
function TopBar({dm,setDm,waOn}){
  ...
  return <div style={{height:56,background:t.surf,borderBottom:`1px solid ${t.bdr}`,
    ...
    <div style={{display:"flex",alignItems:"center",gap:8,fontSize:13,color:t.t2}}>
      Last synced: <span style={{color:t.t1,fontWeight:500}}>2 min ago</span>
    </div>
...
function AISummaryPanel({skus,tickets,audit}){
  ...
  useEffect(()=>{
    ...
    fetchSummary(metrics).then(txt=>{setAiText(txt);setLoading(false);});
  },[]);
  ...
  return <div style={{background:t.surf,borderBottom:`1px solid ${t.bdr}`,
    ...
    {!collapsed&&<div className="fi" style={{padding:"0 20px 12px",display:"flex",gap:16,alignItems:"flex-start"}}>
      <div style={{flex:1,fontSize:13,color:t.t2,lineHeight:1.65,minHeight:60}}>
        {loading?(
          <span style={{color:t.t3,fontStyle:"italic"}}>✦ Generating insights…</span>
        ):(
          (aiText||fallback).split("\n").filter(Boolean).map((line,i)=>(
      ...
      <div style={{fontSize:11,color:t.t3,whiteSpace:"nowrap",paddingTop:4}}>
        Powered by Claude · Avg risk {metrics.avgRisk}/100
      </div>
    </div>}
  </div>;
}
...
function SKUDrawer({sku,onClose,onAction,poOn,waOn,approved,ticketed}){
  ...
  const rec=useMemo(()=>genRec(sku),[sku]);
  ...
  return <div className="dr" style={{position:"absolute",right:0,top:0,height:"100%",
    ...
    <div style={{flex:1,overflowY:"auto",padding:16}}>
      ...
      {/* Recommendation */}
      <div style={{fontSize:11,fontWeight:700,color:t.t3,textTransform:"uppercase",letterSpacing:".05em",marginBottom:8}}>Recommendation</div>
      ...
      {evid&&<div className="fi" style={{background:t.surf2,border:`1px solid ${t.bdr}`,borderRadius:10,padding:12}}>
        {[
          ["competitor_price","Price ₹"+sku.comp?.toLocaleString()+" fetched 4h ago"],
          ["inventory_snapshot",`${sku.onH} units · ${sku.vel}/day → ${sku.doc}d cover`],
          ["demand_stats_7d",`v7=${sku.v7} vs v14=${sku.v14} (${sku.v7>sku.v14?"+":""}${Math.round(((sku.v7-sku.v14)/Math.max(sku.v14,1))*100)}%)`],
          ["margin_calc",`${(sku.mPct*100).toFixed(1)}% margin · floor ${(sku.mFloor*100).toFixed(0)}% · buffer ${Math.round(((sku.mPct-sku.mFloor)/sku.mFloor)*100)}%`],
        ].map(([src,detail])=>(
          <div key={src} style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:8,fontSize:11}}>
...
export default function App(){
  ...
  // Rebuild SKUs when thresholds change
  ...
  return <TC.Provider value={theme}>
    <div style={{display:"flex",height:"100vh",background:theme.bg,overflow:"hidden",position:"relative"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        <TopBar dm={dm} setDm={setDm} waOn={waOn}/>
